...
def window_partition(x, window_size):
    ...
    """
    return rearrange(
        x, "b (h s1) (w s2) c -> (b h w) s1 s2 c", s1=window_size, s2=window_size
    )
...
class BaseBlock(nn.Module):
    def __init__(
        self, dim, num_heads=8, window_size=8, ratios=[1, 2, 2, 4, 4], qkv_bias=False
    ):
...
class MobileSR(nn.Module):
    def __init__(
        self, n_feats=40, n_heads=8, ratios=[4, 2, 2, 2, 4], upscaling_factor=4
    ):
